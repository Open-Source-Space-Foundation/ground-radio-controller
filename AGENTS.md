...
main tests, `./bft.sh 1 fs` to run one-board filesystem tests, and
`./bft.sh 2` to run the two-board tests. The user will have to manually press
a button on each board before it is flashed, so be sure that when you run it,
